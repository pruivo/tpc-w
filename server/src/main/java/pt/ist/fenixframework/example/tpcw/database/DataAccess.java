...
public class DataAccess {
    ...
    private static App getApp() {
        return FenixFramework.getDomainRoot().getApp();
    }

    public static String[] getName(int c_id) {
	Customer customer = getApp().getCustomer(c_id);
	...
	Customer customer = getApp().getCustomer(UNAME);
	if (customer == null) {
	    return null;
	}
	...
	List<Book> bookList = getApp().getBooksBySubjectSortedByTitle(search_key, 50 /*limit*/);
	return convertToBookDTO(bookList);
    }

    public static Vector doTitleSearch(String search_key) {
	List<Book> bookList = getApp().getBooksByTitleSortedByTitle(search_key, 50 /*limit*/);
	return convertToBookDTO(bookList);
    }

    public static Vector doAuthorSearch(String search_key) {
	List<Book> bookList = getApp().getBooksByAuthorSortedByTitle(search_key, 50 /*limit*/);
	return convertToBookDTO(bookList);
    }

    public static Vector getNewProducts(String subject) {
	List<Book> bookList = getApp().getBooksBySubjectSortedByPublicationDateAndTitle(subject, 50 /*limit*/);
	return convertToShortBookDTO(bookList);
    }

    public static Vector getBestSellers(String subject) {
	List<Book> bookList = getApp().getBestSellers(subject,
						       getApp().getNumOrderIds()-3333 /*recent orders*/,
						       50 /*limit*/);
	return convertToShortBookDTO(bookList);
    }
	...
	getApp().updateBookInfo(i_id, cost, image, thumbnail);
    }
	...
	Customer customer = getApp().getCustomer(C_UNAME);
	if (customer == null) {
	    return null;
	} else {
	...
	order_lines.clear();
	for (OrderLine orderLine : order.getOrderLinesSet()) {
	...
	Customer customer = getApp().getCustomer(customer_id);
	if (customer == null) {
	...
	//getCart
	ShoppingCart sc = getApp().getCart(shopping_id);
	...
	shipDate.setNanos(0);
        CCXact ccXact = new CCXact(cc_type, cc_number, cc_name, cc_expiry, total, shipDate, order, shipAddress.getCountry());
        // TxSystem.getManager().save(ccXact);
	order.setCcXact(ccXact);
    }

    public static void clearCart(ShoppingCart cart) {
	cart.getShoppingCartLinesSet().clear();
    }

    public static Address enterAddress(String street1, String street2, String city, String state, String zip, String country) {
	return getApp().findOrCreateAddress(street1, street2, city, state, zip, country);
    }
	...
	return getApp().makeNewOrder(customer, cart, subTotal, total, shipAddr, shipping, c_discount);
    }
	...
	return getApp().makeNewCustomer(customer).getDTO();
    }
}
